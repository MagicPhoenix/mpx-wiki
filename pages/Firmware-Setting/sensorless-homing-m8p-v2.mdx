...
The "Endstop Check" section of that guide will in turn reference the [Setting Up and Calibrating Sensorless XY Homing]( https://docs.vorondesign.com/community/howto/clee/sensorless_xy_homing.html) documentation. You'll need to pick up that guide from the section on [Finding the right StallGuard threshold](https://docs.vorondesign.com/community/howto/clee/sensorless_xy_homing.html#finding-the-right-stallguard-threshold). Note that the section on [Homing Macros](https://docs.vorondesign.com/community/howto/clee/sensorless_xy_homing.html#homing-macros) from that page is already taken care of by the `Sensorless-Homing.cfg` file that we uploaded previously. Also, the steps in the [Final Setup](https://docs.vorondesign.com/community/howto/clee/sensorless_xy_homing.html#final-setup) section have been taken care of already as well: we commented out the `[safe_z_home]` section previously and our `Sensorless-Homing.cfg` file included in `printer.cfg` already contains a `[homing_override]` section.

## One More Thing

The [Voron guide](https://docs.vorondesign.com/community/howto/clee/sensorless_xy_homing.html#final-setup) doesn't mention it, but many people have found that the `driver_SGTHRS` StallGuard sensitivity threshold values need to be a little less sensitive in a heated chamber. So for example, if you found a threshold value of 63 on an axis after going through [the guide](https://docs.vorondesign.com/community/howto/clee/sensorless_xy_homing.html#final-setup) with your chamber at room temperature, you might need to go to 62 or 61 to get reliable operation in a heated chamber. Keep an eye on your homing to start with and if it acts up on one or both axes, you know what to do.

<Callout type="warning">
Any time a `driver_SGTHRS` value is changed in `printer.cfg`, you **must** do a "Firmware Restart" from the Mainsail interface (Power Icon → Firmware Restart) for it to take effect. It is not enough to simply restart Klipper.
</Callout>
