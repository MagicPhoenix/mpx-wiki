...
<Callout type="info">
In the event the Klipper flash fails and the Manta becomes unresponive, it can be recovered by reflashing the bootloader and then klipper, see the torubleshooting section of the initial flashing guide. The USB_5v jumper on the Manta is not needed if the board is powered from the printer.
</Callout>
